...
###   shitcoincore.org and shitcoin.org and compares them.
...
WORKINGDIR="/tmp/shitcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://shitcoincore.org"
HOST2="https://shitcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="shitcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'shitcoin-',
...
if [ $? -ne 0 ]; then
   echo "shitcoin.org failed to provide signature file, but shitcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "shitcoin.org and shitcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Shitcoin Core binary release signing key installed?"
   fi
