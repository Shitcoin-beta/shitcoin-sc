...
const int SHITCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString SHITCOIN_IPC_PREFIX("shitcoin:");
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(SHITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // shitcoin: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(SHITCOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(SHITCOIN_IPC_CONNECT_TIMEOUT);
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    else if (s.startsWith(SHITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // shitcoin: URI
    {
